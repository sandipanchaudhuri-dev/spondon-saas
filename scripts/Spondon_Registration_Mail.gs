...
function doPost(e) {
  try {
    ...
    if (!expectedSecret || data.secret !== expectedSecret) {
      return json_({ success: false, error: 'Unauthorised' });
    }

    validateEmail_(data.email);
    if (MailApp.getRemainingDailyQuota() < 2) {
      throw new Error('Google Mail daily recipient quota is too low for participant and admin notifications');
    }
...
function sendParticipant_(data) {
    ...
    '<div style="margin:0;background:#f5f1ea;padding:24px;font-family:Arial,sans-serif;color:#241d18">' +
      '<div style="max-width:660px;margin:auto;background:#fff;border:1px solid #e4d8c9;border-radius:14px;overflow:hidden">' +
        '<div style="background:#111;color:#fff;padding:24px">' +
          '<div style="font-size:13px;letter-spacing:1px;text-transform:uppercase">Spondon</div>' +
          '<h1 style="margin:8px 0 0;font-size:27px">Sharod Somman 2026</h1>' +
        '</div>' +
        '<div style="padding:26px">' +
          '<p>Dear <strong>' + html_(person) + '</strong>,</p>' +
          '<p>Thank you. We have received the registration for <strong>' + html_(pujo) + '</strong>.</p>' +
          '<div style="background:#f7f3ec;border-left:4px solid #111;padding:16px;margin:20px 0">' +
            '<div style="font-size:12px;text-transform:uppercase;color:#555">Registration reference</div>' +
            '<div style="font-size:22px;font-weight:bold;margin-top:4px">' + html_(reference) + '</div>' +
          '</div>' +
  ...
  MailApp.sendEmail({
    ...
    body: textBody,
    htmlBody: htmlBody,
    name: 'Spondon Sharod Somman 2026',
    replyTo: SPONDON_REPLY_TO
  });
}
...
function sendAdmin_(data) {
  ...
  const details = [
    ['Reference', reference],
    ['Pujo name', data.pujo_name],
    ['Applicant', data.contact_person_name],
    ['Email', data.email],
    ['Phone', data.phone],
  ...
  const htmlRows = details.map(function(row) {
    return '<tr><td style="padding:7px 12px 7px 0;color:#666;vertical-align:top"><strong>' + html_(row[0]) + '</strong></td><td style="padding:7px 0">' + html_(valueOrDash_(row[1])) + '</td></tr>';
  }).join('');
  ...
  MailApp.sendEmail({
    to: String(data.admin_email || SPONDON_ADMIN_EMAIL).trim(),
    subject: subject,
    body: textBody,
    htmlBody: htmlBody,
    name: 'Spondon Registration Notification',
    replyTo: String(data.email || SPONDON_REPLY_TO).trim()
  });
}

function validateEmail_(value) {
  const email = String(value || '').trim();
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error('A valid participant email address is required');
}

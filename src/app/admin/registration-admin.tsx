...
export function RegistrationAdmin({initial}:{initial:Registration[]}){
  ...
  function refresh(){setRefreshing(true);location.reload();}
  ...
  return <main className="admin">
    ...
    <section className="admin-main">
      <div className="topline"><div><div className="eyebrow">Sharod Somman 2026</div><h1>Pujo registrations</h1><p className="muted">{initial.length} registration{initial.length===1?"":"s"} in the live Spondon database.</p></div><button className="button secondary" onClick={refresh} disabled={refreshing}>{refreshing?"Refreshing…":"Refresh registrations"}</button></div>
      <section className="card table-card" id="registrations">
        <div className="toolbar" style={{gap:12,flexWrap:"wrap"}}>
          ...
          <button className="button secondary" onClick={refresh} disabled={refreshing}>{refreshing?"Refreshing…":"Refresh"}</button>
